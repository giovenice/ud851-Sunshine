...
//      TODO (7) Generate a weather summary with the date, description, high and low
        ...
        String data = SunshineDateUtils.getFriendlyDateString(mContext,dateInMillis,false);
        Double high = mCursor.getDouble(MainActivity.INDEX_WEATHER_HIGH);
        Double low =  mCursor.getDouble(MainActivity.INDEX_WEATHER_LOW);
        String high_n_low = SunshineWeatherUtils.formatHighLows(mContext,high,low);
        int descID = mCursor.getInt(MainActivity.INDEX_WEATHER_DESCRIPTION);
        String description = SunshineWeatherUtils.getStringForWeatherCondition(mContext, descID);
        String summary = data + description + high_n_low;
